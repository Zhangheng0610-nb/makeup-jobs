...
4. **字段填写规范（硬性，构建器靠这些字段计算"达标"）**：
   ...
   - 📝 **指路**（可选字段，**硬性：必须真实可达**）：**没有具体岗位帖但岗位真实且值得收录时**才写——写明在哪个平台搜什么关键词（如「58同城搜索'蓝佳美甲 化妆师'查看原帖」），让求职者能自己找到；有合格链接就不用写。**写指路前必须用搜索结果核实该平台确实存在该公司的岗位**（能搜到公司名/岗位名才算数）；若多个平台都查证不到任何在招记录 → **整条删除，禁止虚指**（用户实测搜不到会失去信任；宁可少一条，不可指一条死路）
   - **达标由构建器按薪资自动计算**（底薪+提成估算合计 ≥8000元/月），你只负责如实填薪资文本，**不要在标题里标✅**
5. **质量门槛与写文件时机（硬性）**：
   - 先按**扩大搜索矩阵**搜 18-25 个查询：区县（鄞州/海曙/江北/镇海/北仑/奉化/慈溪/余姚/宁海/象山）× 平台（58/智联/前程无忧/全职网/店长直聘/全影/象山明聘/宁波招聘网/小红书/抖音）× 岗位（化妆师/彩妆/造型/跟妆/儿童/助理/摄影师）。**每次更新必须换新关键词、扩大覆盖面**（可加"2026年8月""最新""新店""高薪""诚聘"等变化词），禁止只复用上次的查询；搜到新公司/新平台继续深挖该公司其他岗位
